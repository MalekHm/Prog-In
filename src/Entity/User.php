...
class User
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=Reservationproduct::class, mappedBy="idclient")
     */
    private $id_user;
    ...
    public function addIdUser(Reservationproduct $idUser): self
    {
        if (!$this->id_user->contains($idUser)) {
            $this->id_user[] = $idUser;
            $idUser->setIdclient($this);
        }

        return $this;
    }

    public function removeIdUser(Reservationproduct $idUser): self
    {
        if ($this->id_user->removeElement($idUser)) {
            // set the owning side to null (unless already changed)
            if ($idUser->getIdclient() === $this) {
                $idUser->setIdclient(null);
            }
        }
